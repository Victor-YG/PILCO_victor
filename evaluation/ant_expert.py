...
def main():
    ...
    for i in range(args.iterations):
        ...
        # update dataset
        X = np.vstack((X, X_new[:, 0 : state_dim]))
        U = np.vstack((U, U_new[:, 0 : state_dim]))
        E = np.vstack((E, E_new[:, 0 : state_dim]))
        Y = np.vstack((Y, Y_new[:, 0 : state_dim]))
        end_time = time.time()
